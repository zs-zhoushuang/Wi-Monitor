...
class Seg_network():
    def __init__(
        ...
        if is_train is False:
            ...
            self.saver.restore(self.sess,"./Params_cross_environment_scenario/cross_envir_1_2/tf_1_2/params_200/train.ckpt")
            print('load params sucess')
            print('0.5:')
            self.test_Seg(self.test_data, 'person in trainset test', 0.50)
            self.test_Seg(self.test_no_trainSet_data, 'person not in trainset test', 0.50)
            print('0.75:')
            self.test_Seg(self.test_data, 'person in trainset test', 0.75)
    ...
    def build(self):
            ...
            with tf.variable_scope('train'):
                ...
                self.optimizer = tf.train.AdamOptimizer(self.learning_rate).minimize(self.loss,var_list=self.fune_turn_list)
    ...
    def TemporalBlock(self,input_layer,out_channels, filter_size, stride, dilation_rate, counters,
        ...
        with tf.variable_scope(name):
            ...
            self.fune_turn_list.append(W_h)
            self.fune_turn_list.append(b_h)
    ...
    def test_Seg(self,input_data,str1,f1_param):
        ...
        # with open('result/tf/envir_1_3/'+str1+'.kpl','wb') as handle:
...
if __name__ =="__main__":
    ...
    for person_index in range(1,5,1):
        for list_index in range(1,3,1):
            path = 'TestData_cross_environment_scenario/envir_2/person_' + str(person_index) + '/list_'+str(list_index)+'/'
