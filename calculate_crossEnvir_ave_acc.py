    ...
    for m in range(0,2):
        ...
        for i in range(2, 4, 1):
            TP, FP, FN, correct_count, test_sum = test_Ave_Seg(
                'ave_accuracy/cross_envir_1_to_' + str(i) +  '/person in trainset test', 20, f1_param)
            all_TP += TP
            all_FP += FP
            all_FN += FN
            all_correct_count += correct_count
            sum += test_sum
        print('cross_envir person in trainset test:')
        ...
        for i in range(2, 4, 1):
            TP, FP, FN, correct_count, test_sum = test_Ave_Seg(
                'ave_accuracy/cross_envir_1_to_' + str(i) + '/person not in trainset test', 20, f1_param)
            all_TP += TP
            all_FP += FP
            all_FN += FN
            all_correct_count += correct_count
            sum += test_sum
        print('cross_envir person not in trainset test:')
        print('average package-wise accuracy is :', all_correct_count / sum)
        # print('all_TP,all_FP,all_FN ',all_TP,all_FP,all_FN)
        precision = all_TP / (all_TP + all_FP)
        recall = all_TP / (all_TP + all_FN)
